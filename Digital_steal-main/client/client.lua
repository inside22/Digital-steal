        ...
        for _, steal in pairs(data_steal) do
            if steal.steal_source ~= 0 and steal.steal_enable then
                ...
                if PromptHasHoldModeCompleted(StealPrompt[steal.steal_ped]) then
                    ...
                    TriggerServerEvent('digital_steal:MoneyOpenMenu', steal.steal_source)
                end
            elseif active_menu and steal_source == steal.steal_source then
                ClearPedTasks(PlayerPedId())
                TriggerServerEvent('digital_steal:CloseInventory')
                active_menu = false
                MenuData.CloseAll()
            end
        end
...
RegisterNetEvent('digital_steal:StealingPlayers')
AddEventHandler('digital_steal:StealingPlayers', function(source)
    if source ~= steal_source then
        table.insert(stealing_players, source)
    end
end)

RegisterNetEvent('digital_steal:DelStealingPlayers')
AddEventHandler('digital_steal:DelStealingPlayers', function(source)
...
RegisterNetEvent('digital_steal:OpenMenu')
AddEventHandler('digital_steal:OpenMenu', function(money)
    MenuData.CloseAll()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi', {
        ...
        if (data.current.value == 'inventory') then
            TriggerServerEvent('digital_steal:ReloadInventory', steal_source)
            TriggerServerEvent('digital_steal:OpenInventory', steal_source)
            Wait(500)
        end
        ...
        active_menu = false
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('digital_steal:CallDelStealingPlayers', steal_source)
        steal_source = nil
    end)
end)

RegisterNetEvent('digital_steal:GetSourceSteal')
AddEventHandler('digital_steal:GetSourceSteal', function(obj, option)
    if option == 'move' and steal_source then
        TriggerServerEvent('digital_steal:MoveTosteal', obj, steal_source)
    elseif option == 'take' and steal_source then
        TriggerServerEvent('digital_steal:TakeFromsteal', obj, steal_source)
    end
end)
